...
namespace WPFormsTribe\Vendor\GuzzleHttp\Handler;

use WPFormsTribe\Vendor\GuzzleHttp\Exception\ConnectException;
use WPFormsTribe\Vendor\GuzzleHttp\Exception\RequestException;
use WPFormsTribe\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use WPFormsTribe\Vendor\GuzzleHttp\Promise\PromiseInterface;
use WPFormsTribe\Vendor\GuzzleHttp\Psr7;
use WPFormsTribe\Vendor\GuzzleHttp\TransferStats;
use WPFormsTribe\Vendor\GuzzleHttp\Utils;
...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return \WPFormsTribe\Vendor\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $headers = \WPFormsTribe\Vendor\GuzzleHttp\headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \WPFormsTribe\Vendor\GuzzleHttp\Promise\rejection_for($ex);
            }
        }
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \WPFormsTribe\Vendor\GuzzleHttp\normalize_header_keys($headers);
    ...
    private function add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\WPFormsTribe\Vendor\GuzzleHttp\is_host_in_noproxy($request->getUri()->getHost(), $value['no'])) {
    ...
    private function add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \WPFormsTribe\Vendor\GuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \WPFormsTribe\Vendor\GuzzleHttp\debug_resource($value);
